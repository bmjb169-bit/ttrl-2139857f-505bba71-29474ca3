...
def main():
    ...
    def save_ckpt(tag):
        ...
        model.config.use_cache = False
        print(f"[ckpt] saved ({tag}) -> {args.out}", flush=True)

    log = []
    G = args.group_size
    saved_any = False
    for step in range(args.steps):
        # Whole step is guarded: a transient CUDA OOM / bad generation on one step
        # must NOT kill the whole run (this is what crashed earlier ~step 3-4).
        try:
            ...
            _g0 = _t.time()
            model.eval()
            # Keep rollout sequences on CPU during the update phase so the GPU
            # isn't holding 64 long sequences alongside the training graph.
            all_seq_cpu, all_promptlen, group_rewards, group_meta = [], [], [], []
            with torch.no_grad():
                for pi, ptext in enumerate(prompts):
                    ...
                    for gi in range(G):
                        all_seq_cpu.append(gen[gi].detach().to("cpu"))
            ...
            n_active = sum(1 for a in advs if abs(a) >= 1e-8)
            denom_n = max(1, n_active)
            for idx in range(len(all_seq_cpu)):
                adv = advs[idx]
                if abs(adv) < 1e-8:
                    continue
                try:
                    seq = all_seq_cpu[idx].unsqueeze(0).to(dev)
                    ...
                    loss = (pg + args.kl_coef * kl) / denom_n
                    loss.backward()
                    total_loss += pg.item(); total_kl += kl.item(); ntok += 1
                    del out, logits, logp, seq, attn
                except torch.cuda.OutOfMemoryError:
                    print(f"[step {step}] OOM on seq {idx}; skipping it", flush=True)
            ...
            # Periodic save so a partial run (instance time limit / later crash)
            # still yields an evaluable TTRL checkpoint.
            if (step + 1) % 5 == 0 or step == args.steps - 1:
                save_ckpt(f"step {step}")
                saved_any = True
        except torch.cuda.OutOfMemoryError:
            print(f"[step {step}] step-level OOM; clearing cache and continuing", flush=True)
            opt.zero_grad(set_to_none=True)
            torch.cuda.empty_cache()
            continue
        except Exception as e:
            import traceback
            print(f"[step {step}] step failed ({type(e).__name__}: {e}); skipping", flush=True)
            traceback.print_exc()
            opt.zero_grad(set_to_none=True)
            torch.cuda.empty_cache()
            continue

    # Final save (idempotent; ensures a checkpoint exists even if no periodic save fired)
    save_ckpt("final")
