...
# FAST_TRAIN=1 skips the smoke gate AND the baseline eval, jumping straight to
# TTRL training. Default OFF here: this is a FRESH project with no prior runs, so
# the first run measures everything self-contained — smoke gate (qwen3_5 serves
# in vLLM), the REAL MATH-L1 baseline, then TTRL training, then re-eval on the
# same test set. That makes the before/after margin fully credible in-project.
...
run_eval () {  # $1 = model path/id, $2 = label, $3 = out json
  # HF-based eval (NOT vLLM): the trained ckpt is saved with Qwen3_5TextConfig,
  # which vLLM's qwen3_5 loader rejects. AutoModelForCausalLM loads BOTH the base
  # and the trained ckpt, so base & ttrl are scored by the identical engine ->
  # strictly comparable. n=8 / 768 tok keeps two HF evals within the time budget.
  $PY -u scripts/ttrl_eval_hf.py --model "$1" --label "$2" --out "$3" \
      --data "$TEST_JSON" --n 8 --max-tokens 768 --temperature 0.6 2>&1 | tail -8
}
